...
class TestSolverPipeline(unittest.TestCase):
    ...
    def test_run_case_tangent_cone_changes_windward_magnitude(self):
        with tempfile.TemporaryDirectory(prefix="newtsolver_test_") as td:
            row_newtonian = {
                "case_id": "test_case_newtonian_tc",
                "stl_path": "samples/stl/cube.stl",
                "stl_scale_m_per_unit": 1.0,
                "alpha_deg": 0.0,
                "beta_or_bank_deg": 0.0,
            ...
            row_tangent_cone = {
                **row_newtonian,
                "case_id": "test_case_tangent_cone",
                "windward_eq": "tangent_cone",
            }

            result_n = run_case(row_newtonian, lambda _msg: None)
            result_tc = run_case(row_tangent_cone, lambda _msg: None)

            self.assertGreater(float(result_tc["CA"]), 0.0)
            self.assertLess(float(result_tc["CA"]), float(result_n["CA"]))
    ...
    def test_run_cases_chunk_callback(self):
        with tempfile.TemporaryDirectory(prefix="newtsolver_test_") as td:
            ...
            self.assertEqual(chunks, [(2, 3, False, 2), (3, 3, True, 3)])
    ...
    def test_parallel_checkpoint_csv_keeps_input_order(self):
        ...
        with tempfile.TemporaryDirectory(prefix="newtsolver_checkpoint_") as td, patch(
            "newtsolver.core.solver.iter_case_results_parallel",
            ...
            run_cases(
                ...
                chunk_cb=lambda chunk_df, done, total, is_final: (
                    write_results_csv(str(out_csv), df, chunk_df),
                    chunks.append(
                        (
                            done,
                            total,
        ...
        self.assertEqual(
            chunks,
            [
                (2, 3, False, ["case_b", "case_c"]),
                (3, 3, True, ["case_a", "case_b", "case_c"]),
            ],
        )
