...
try:
    ...
    from newtsolver.app.viewer import (
        ViewerPanel,
...
from newtsolver.core.solver import build_case_signature
...
class TestViewerVtpMatching(unittest.TestCase):
    ...
    def test_batch_image_export_skips_stale_vtp(self):
        ...
        with tempfile.TemporaryDirectory(prefix="newtsolver_viewer_") as td:
            with (
                patch.object(QtWidgets.QFileDialog, "getExistingDirectory", return_value=td),
                patch("newtsolver.app.viewer.pv.read", return_value=poly),
