...
try:
    ...
    from newtsolver.app.ui_cases import CasesPanel
...
from newtsolver.io.io_cases import InputValidationError, ValidationIssue
...
class TestCasesPanelState(unittest.TestCase):
    ...
    def test_pick_input_file_clears_old_state_after_validation_error(self):
        ...
        with tempfile.TemporaryDirectory(prefix="newtsolver_ui_cases_") as td:
            with (
                patch.object(
                ...
                patch(
                    "newtsolver.app.ui_cases.read_cases",
                    side_effect=InputValidationError(issues),
                ),
                patch("newtsolver.app.ui_cases._ValidationIssuesDialog.exec", return_value=0),
            ):
